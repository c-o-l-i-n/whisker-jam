...
export function unmute(
  ...
  forceIOSBehavior: boolean = false,
): { dispose: { (): void } } {
  //#region Helpers
  ...
  if (document.hidden !== undefined)
    ...
    pageVisibilityAPI = {
      hidden: <any>'webkitHidden',
      visibilitychange: <any>'webkitvisibilitychange',
    };
  else if ((<any>document).mozHidden !== undefined)
    pageVisibilityAPI = {
      hidden: <any>'mozHidden',
      visibilitychange: <any>'mozvisibilitychange',
    };
  else if ((<any>document).msHidden !== undefined)
    pageVisibilityAPI = {
      hidden: <any>'msHidden',
      visibilitychange: <any>'msvisibilitychange',
    };
  ...
  function removeEventListeners(
    ...
    for (let i: number = 0; i < events.length; ++i)
      target.removeEventListener(events[i], handler, <any>{
        capture: capture,
        passive: passive,
      });
  }
  ...
  let allowPlayback: boolean = true; // Assume page is visible and focused by default
  ...
  // @ts-expect-error: idk why this throws an error, but the code works
    ...
    if (allowPlayback) {
      ...
      if (context.state !== 'running' && context.state !== 'closed') {
        ...
        if (hasMediaPlaybackEventOccurred) {
          const p: Promise<any> = context.resume();
      ...
      if (context.state === 'running') {
        const p: Promise<any> = context.suspend();
        if (p) p.then(noop, noop).catch(noop);
      }
    }
  }
   ...
   * @param evt The event.
   */
  function context_statechange(evt: Event): void {
    // Check if the event was already handled since we're listening for it both ways
    if (!evt || !(<any>evt).unmute_handled) {
      // Mark handled
      (<any>evt).unmute_handled = true;
      ...
      if (allowPlayback) {
        ...
        if (isMediaPlaybackEvent) {
          ...
          if (!channelTag) {
            ...
            (<any>channelTag).disableRemotePlayback = true; // Airplay like controls on other devices, prevents casting of the tag, doesn't work on modern iOS
  ...
  let hasMediaPlaybackEventOccurred: boolean = false;
